...
class NERLearner(object):
    ...
    def predict_batch(self, words):
        ...
        if self.use_elmo:
            ...
            inputs = (word_input);#print(inputs)
            ...
            inputs = (word_input, char_input);#print(inputs)
        ...
        prediction = [p[:i] for p, i in zip(predictions, sequence_lengths)];
        
        return prediction

    # Tag tokens with standard NLP BIO tags
    def my_bio_tagger(self, ne_tagged):
       bio_tagged = []
       prev_tag = "O"
    ...
    def predict(self, sentences):
        ...
        doc = nlp(sentences)
        words_raw = [[token.text for token in sent] for sent in doc.sents];#print(words_raw)
        if self.use_elmo:
            words = words_raw
        else:
            ...
            #     words = zip(*words)
        pred_ids = self.predict_batch(words);#print(pred_ids)
        preds = [[self.idx_to_tag[idx.item() if isinstance(idx, torch.Tensor) else idx]  for idx in s] for s in pred_ids]
        
        #return pred for normal output
        preds1=list(more_itertools.collapse(preds));#print(preds1)
        words1=list(more_itertools.collapse(words_raw));#print(words1)
        input_text = ' '.join(words1);#print(input_text)
        input_text = input_text + " "
        
        # return for displacy format
        bio_tagged = []
        end=-1
        for i in range(len(preds1)):
            start = end+1
            end = start + len(words1[i])
            if(preds1[i]!="O"):
                dict1={"start": start, "end": end, "label": preds1[i]}
                bio_tagged.append(dict1)

        normal_tagged=[]
        j=0
        for i in range(len(bio_tagged)):
            if(bio_tagged[i].get('label').find('I-')!=-1):
                prev = normal_tagged[j-1]
                if(prev.get('label').find('B-')!=-1):
                    prev['end']=bio_tagged[i].get('end');
                else:
                    normal_tagged.append(bio_tagged[i])
                    j+=1
            else:
                normal_tagged.append(bio_tagged[i])
                j+=1

        my_defined_tags = {"I-FRE":"FREQUENCY", "B-FRE":"FREQUENCY", "B-FOR":"FORM", "I-FOR":"FORM", "I-ADE":"ADE", "B-ADE":"ADE", "B-STR":"STRENGTH", "I-STR":"STRENGTH", "B-REA":"REASON", "I-REA":"REASON", "B-DUR":"DURATION", "I-DUR":"DURATION", "I-DOS":"DOSAGE", "B-DOS":"DOSAGE", "B-ROU":"ROUTE", "I-ROU":"ROUTE", "B-DRUG":"DRUG", "I-DRUG":"DRUG"}
        for i in range(len(normal_tagged)):
            normal_tagged[i]['label']=my_defined_tags[normal_tagged[i]['label']]

        #print("\n\n\n");print(normal_tagged);print("\n\n\n");
                
        output_data=[{"text": input_text, "ents": normal_tagged, "title": None}]

        #dict format
        final_dict=[]
        
        s = bio_tagged[0].get('start')
        if(s!=0):
            txt = input_text[0:s]
            dict1={'data': txt,'label':'O'}
            final_dict.append(dict1)
        prev=s
        for i in range(len(normal_tagged)):
            s = normal_tagged[i].get('start')
            e = normal_tagged[i].get('end')
            l = normal_tagged[i].get('label')
            if(s==prev):
                txt = input_text[s:e]
                dict1={'data': txt,'label': l}
                final_dict.append(dict1)
            else:
                txt = input_text[prev:s]
                dict1={'data': txt,'label': 'O'}
                final_dict.append(dict1)
                txt = input_text[s:e]
                dict1={'data': txt,'label': l}
    ...
    def predict_pdf(self, sentences):
        """Returns list of tags
